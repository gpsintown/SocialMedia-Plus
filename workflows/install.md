...
4. Run `python3 scripts/smp-start.py --root .` using the same root as the installer. Open the local dashboard returned by startup, normally `http://127.0.0.1:4010`. Configure the LinkedIn app values using its setup form and follow the LinkedIn setup guide. Never paste tokens into a public issue, chat log or committed file. Saving environment variables is not an OAuth grant or successful publishing test.
